...
const AddMenuModal = ({open, setOpen}) => {
	...
	const handleChangeLink = useCallback((event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
	...
	const isAddLinkDisabled = Boolean(linkPlaceHolder.title === '' || linkPlaceHolder.href === '')
	const isAddMenuDisabled = Boolean(menuData?.title === '')
	const handleDeleteLink = useCallback((index: number) => {
		const filteredLinks = menuData.links.filter((item, innerIndex) => innerIndex !== index)
		setMenuData(prevState => ({
			...prevState,
			links: filteredLinks,
		}))
